...
var ACTIONS = []string{"run", "confirm", "discard", "cancel", "cleanup", "echo","createprojectworkspace"}
...
func main() {
	...
	project := flag.String("project", "", "Workspace project (optional)")
	search := flag.String("search", "", "Workspace search (optional)")
	action := flag.String("action", "", "Action to do on the Workspace(s) [run|confirm|discard|cancel|cleanup|echo|createprojectworkspace] (required)")
	...
	case "createprojectworkspace":
		client.CreateProjectWorkspace(ctx, *org, *project, *search)
	}
...
func (c *Client) Echo(ctx context.Context, org, search string) error {
	...
	for _, ws := range workspaces {
		slog.Info("found", "workspace", ws.Name, "runID", ws.CurrentRun.ID, "status", ws.CurrentRun.Status,  "workspaceID", ws.ID, "TerraformVersion", ws.TerraformVersion)
	}
...
// 
func (c *Client) CreateProjectWorkspace(ctx context.Context, org, proj string, search string) error {
	project, err := c.Projects.Create(ctx, org, tfe.ProjectCreateOptions{
		Name:          proj,
	})
	if err != nil {
		return err
	...
	workspaces, err := c.Workspaces.Create(ctx, org, tfe.WorkspaceCreateOptions{
		Name:                       tfe.String(search),
	...
	slog.Info("found", "workspace", workspaces.Name, "runID", workspaces.CurrentRun.ID, "status", workspaces.CurrentRun.Status, "workspaceID", workspaces.ID, "TerraformVersion", workspaces.TerraformVersion, "Project", workspaces.Project.Name)
